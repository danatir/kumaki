...
const WORD_LEVELS = ['GEN','L1','L2','L3','L4','L5','L6','L7','L8','L9','L10','L11','L12','Q'];
...
function kanjiCardHTML(k, kl){
  ...
  const part = k.part ? `<span class="vc-sub-tag">${kl.replace('KL','')}-${k.part}</span>` : '';
...
function renderWords(){
  ...
  for(const lvl of WORD_LEVELS){
    const kl = /^L\d+$/.test(lvl) ? 'KL'+lvl.slice(1) : null;
    ...
    html += `<div class="level-tag"><span class="lt-key">${lvl}${kl&&allK.length?' · '+kl:''}</span><span class="lt-sub notranslate" translate="no">${meta.jp}</span><span class="lt-en">${meta.en}</span><span class="lt-line"></span><span class="lt-count">${counts.join(' · ')}</span></div>`;
